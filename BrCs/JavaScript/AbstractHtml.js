...
function AbstractHtmlPost(postContainer = "postcontainer") {
    ...
    document.getElementsByClassName(postContainer)[0].innerHTML = '' +
        ...
        '<img src="' + PathSvgHome + 'sezione9-2.svg" class="image-9" v-on:click="ClickCalps(post.id)"/>' +
        '<div class="applausinumero">{{ post.claps }}</div>' +
        '<img src="' + PathSvgHome + 'sezione8.svg" class="image-10" v-on:click="ClickLike(post.id)"/>' +
        ...
        '</div>' +
        '<div class="div-block-4">' +
        '<p class="paragraph" v-for="comment in post.comments">' +
        ...
        '<img src="' + PathSvgHome + 'sezione7.svg" class="image-8" v-on:click="ClickComment(post.id)"/>' +
        '</form>' +
    ...
    let posts = new Vue({
        ...
        methods: {
            ClickCalps: function (idPost) {
                console.log("click post claps idPost: " + idPost);
            },
            ClickLike: function (idPost) {
